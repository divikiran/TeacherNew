...
using NPAInspectionWriter.iOS.IO;
using UIKit;
...
namespace NPAInspectionWriter.Services
{
    ...
    internal class MediaPickerDelegate : UIImagePickerControllerDelegate
    {
        ...
        private MediaFile GetPictureMediaFile( NSDictionary info )
        {
            ...
            image = image.ScaleAndRotateImage( UIImageOrientation.Up );
            ...
            var sanitizedPath = path.RemoveApplicationRoot();
            return new MediaFile( sanitizedPath, () => File.OpenRead( path ), dispose );
        }
